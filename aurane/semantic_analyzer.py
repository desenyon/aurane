...
ACTIVATIONS = {
    ...
    'silu', 'mish', 'elu', 'selu', 'swish', 'hardswish',
...
LOSS_FUNCTIONS = {
    'cross_entropy', 'mse', 'mae', 'bce', 'nll', 'huber',
...
class SemanticAnalyzer:
    ...
    def _collect_definitions(self):
        ...
        for model in self.program.models:
            if model.name in self.defined_models:
                self._add_issue(
                    IssueKind.ERROR, f"Duplicate model definition: {model.name}",
                    f"model {model.name}", "E001"
                )
        ...
        for dataset in self.program.datasets:
            if dataset.name in self.defined_datasets:
                self._add_issue(
                    IssueKind.ERROR, f"Duplicate dataset definition: {dataset.name}",
                    f"dataset {dataset.name}", "E002"
                )
        ...
        for exp in self.program.experiments:
            if exp.name in self.defined_experiments:
                self._add_issue(
                    IssueKind.WARNING, f"Duplicate experiment definition: {exp.name}",
                    f"experiment {exp.name}", "W001"
                )
    ...
    def _analyze_imports(self):
        ...
        for use in self.program.uses:
            if use.module in imported_modules:
                self._add_issue(
                    IssueKind.WARNING, f"Duplicate import: {use.module}",
                    f"use {use.module}", "W002"
                )
                ...
                if isinstance(batch, int):
                    if batch <= 0:
                        self._add_issue(
                            ...
                            f"dataset {dataset.name}", "E003"
                        )
                    elif batch > 1024:
                        self._add_issue(
                            IssueKind.SUGGESTION,
                            f"Large batch size ({batch}) may cause memory issues",
                            f"dataset {dataset.name}", "S002"
                        )
    ...
    def _analyze_model(self, model: ModelNode):
        ...
        if not model.forward_block:
            self._add_issue(
                IssueKind.ERROR,
                "Model has no forward block",
                f"model {model.name}", "E004"
            )
            return
        ...
        if not ops:
            self._add_issue(
                IssueKind.WARNING,
                "Model has empty forward block",
                f"model {model.name}", "W006"
            )
            return
    ...
    def _analyze_operation(self, op: LayerOperation, model_name: str, idx: int):
        ...
        if op_name not in LAYER_SPECS:
            self._add_issue(
                IssueKind.WARNING,
                f"Unknown operation: {op.operation}",
                location, "W007"
            )
            return
        ...
        if op.activation:
            ...
            if activation not in ACTIVATIONS:
                self._add_issue(
                    IssueKind.WARNING,
                    f"Unknown activation: {op.activation}",
                    location, "W008"
                )
            elif spec['activations'] and activation not in spec['activations'] and activation != 'residual':
                self._add_issue(
                    IssueKind.INFO,
            ...
            if name == 'conv2d':
                if i + 1 < len(ops) and op_names[i + 1] != 'batchnorm':
                    self._add_issue(
                        ...
                        f"model {model.name}, layer {i}", "S004"
                    )
        ...
        if ops and ops[-1].operation.lower() in ('dense', 'linear'):
            if ops[-1].activation in ('relu', 'gelu'):
                self._add_issue(
                    ...
                    f"model {model.name}, output", "S005"
                )
    ...
    def _analyze_training(self):
        ...
        for train in self.program.trains:
            ...
            if train.model_name not in self.defined_models:
                self._add_issue(
                    IssueKind.ERROR,
                    f"Undefined model: {train.model_name}",
                    location, "E006"
                )
            ...
            if train.dataset_name not in self.defined_datasets:
                self._add_issue(
                    IssueKind.ERROR,
                    f"Undefined dataset: {train.dataset_name}",
                    location, "E007"
                )
            ...
            if optimizer:
                opt_name = optimizer.split('(')[0].lower() if '(' in optimizer else optimizer.lower()
    ...
    def _suggest_improvements(self):
        ...
        if not self.program.models and not self.program.datasets:
            self._add_issue(
                IssueKind.WARNING,
                "Program appears to be empty",
                "program", "W012"
            )
    
    def _add_issue(self, kind: IssueKind, message: str, location: str, 
                   code: str, fix: Optional[str] = None):
        """Add an issue to results."""
        self.result.issues.append(SemanticIssue(
            kind=kind,
            message=message,
            location=location,
            code=code,
