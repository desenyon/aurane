...
[bold cyan]   ╚═╝  ╚═╝ ╚═════╝ ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═══╝╚══════╝[/bold cyan]
   ...
   [dim]Version 0.2.0 • PyTorch Backend • MIT License[/dim]
    """
...
def cmd_compile_enhanced(args):
    ...
    try:
        ...
        table.add_row("[green]✓ Status", "[green bold]Success")
        table.add_row("Input", f"[dim]{args.input}[/dim]")
        table.add_row("Output", f"[dim]{args.output}[/dim]")
        table.add_row("Input Size", f"[cyan]{input_stats['lines']}[/cyan] lines, [cyan]{input_stats['size']:,}[/cyan] bytes")
        table.add_row("Output Size", f"[cyan]{output_stats['lines']}[/cyan] lines, [cyan]{output_stats['size']:,}[/cyan] bytes")
        ...
        if args.analyze:
            ...
            # Count layers
            total_layers = sum(len(model.forward_block.operations) if model.forward_block else 0 
...
def cmd_inspect(args):
    ...
    try:
        ...
        if args.export:
            ...
            console.print(f"\n[green]✓[/green] Exported AST to {args.export}")
...
def show_statistics(ast):
    ...
    stats_table.add_row("Experiments", str(len(ast.experiments)), ", ".join(e.name for e in ast.experiments))
        ...
        def on_modified(self, event):
            if event.src_path == str(input_path.absolute()):
                ...
                if console is not None:
                    console.print(f"\n[yellow]⟳ File changed, recompiling...[/yellow]")
...
def cmd_format(args):
    ...
    try:
        files = list(Path(args.path).rglob('*.aur')) if Path(args.path).is_dir() else [Path(args.path)]
        ...
        for file in files:
            ...
            if original != formatted:
                if not args.check:
                    ...
                    console.print(f"[green]✓[/green] Formatted {file}")
                else:
    ...
    try:
        ...
        for i, line in enumerate(lines, 1):
            ...
            if line and line[0] == ' ':
                ...
                if spaces % 4 != 0:
                    issues.append(('warning', f"Line {i}: Inconsistent indentation ({spaces} spaces)"))
...
def cmd_benchmark(args):
    ...
    try:
        ...
        console.print(f"[dim]Throughput: {file_stats['lines'] / (stats(times['total'])['mean']):.0f} lines/sec[/dim]")
                ...
                if cmd_name in ('exit', 'quit', 'q'):
                    ...
                    if not cmd_args:
                        ...
                        if code_buffer:
                            ...
                            console.print(f"[green]✓[/green] Saved to {cmd_args}")
                        else:
                            console.print("[yellow]Buffer is empty[/yellow]")
                    ...
                    if not cmd_args:
                        ...
                        try:
                            ...
                            console.print(f"[green]✓[/green] Loaded {len(code_buffer)} lines from {cmd_args}")
                        except FileNotFoundError:
                            console.print(f"[red]✗ Error:[/red] File not found: {cmd_args}")
                    ...
                    if code_buffer:
                        ...
                        try:
                            ...
                            if errors:
                                ...
                                console.print("[green]✓ Valid Aurane code[/green]")
                        except Exception as e:
                    ...
                    console.print(f"[red]Unknown command:[/red] {cmd_name} (type .help for commands)")
...
Examples:
  ...
  aurane interactive
    ...
    compile_parser.add_argument('--backend', default='torch', choices=['torch'], help='Code generation backend')
    compile_parser.add_argument('--analyze', action='store_true', help='Show analysis after compilation')
    ...
    inspect_parser.add_argument('--verbose', '-v', action='store_true', help='Show detailed information')
    ...
    watch_parser.add_argument('--backend', default='torch', choices=['torch'], help='Code generation backend')
    watch_parser.add_argument('--analyze', action='store_true', help='Show analysis on each compile')
    ...
    benchmark_parser.add_argument('--iterations', '-n', type=int, default=10, help='Number of iterations')
    ...
    run_parser.add_argument('--backend', default='torch', choices=['torch'], help='Code generation backend')
