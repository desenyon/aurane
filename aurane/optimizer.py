...
class ASTOptimizer:
    ...
    def optimize(self, level: int = 1) -> OptimizationResult:
        ...
        self.stats['original_layers'] = sum(
            len(m.forward_block.operations) if m.forward_block else 0
        ...
        return OptimizationResult(
            ...
            applied_optimizations=self.applied,
            stats=self.stats
        )
    ...
    def _fuse_conv_bn_relu(self):
        ...
        for model in self.program.models:
            ...
            while i < len(ops):
                ...
                if (op.operation.lower() == 'conv2d' and 
                    i + 1 < len(ops) and 
                    ops[i + 1].operation.lower() == 'batchnorm'):
                    ...
                    if (i + 2 < len(ops) and 
                        ops[i + 2].operation.lower() in ('relu', 'gelu')):
    ...
    def _remove_redundant_activations(self):
        ...
        for model in self.program.models:
            ...
            for i, op in enumerate(ops):
                ...
                if (op.operation.lower() in ('relu', 'gelu', 'sigmoid', 'tanh') and
                    i > 0 and new_ops[-1].activation):
    ...
    def _optimize_dropout_placement(self):
        ...
        for model in self.program.models:
            ...
            if len(ops) >= 2:
                ...
                if (ops[-1].operation.lower() in ('dense', 'linear') and
                    ops[-2].operation.lower() == 'dropout'):
    ...
    def _fuse_consecutive_dense(self):
        ...
        for model in self.program.models:
            ...
            while i < len(ops):
                ...
                if (op.operation.lower() in ('dense', 'linear') and
                    not op.activation and
                    i + 1 < len(ops) and
                    ops[i + 1].operation.lower() in ('dense', 'linear')):
    ...
    def _optimize_pooling(self):
        ...
        for model in self.program.models:
            ...
            for i, op in enumerate(ops):
                ...
                if (op.operation.lower() == 'maxpool' and
                    i + 1 < len(ops) and
                    ops[i + 1].operation.lower() == 'maxpool'):
